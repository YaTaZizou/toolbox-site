  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 mb-6">
        ...
        >
          {loading ? "Génération en cours..." : "✨ Générer des pseudos"}
        </button>
      </div>
        ...
        <div className="space-y-3">
          ...
          {results.map((pseudo, i) => (
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 flex items-center justify-between hover:border-gray-600 transition-colors">
              ...
              <button
                onClick={() => copy(pseudo)}
                className="text-sm text-gray-500 hover:text-purple-400 transition-colors ml-4 flex-shrink-0"
              >
          ...
          <button
            onClick={generate}
            className="w-full mt-4 border border-gray-700 hover:border-gray-500 text-gray-400 hover:text-white py-3 rounded-xl transition-colors text-sm"
          >
            🔄 Régénérer
          </button>
        </div>
